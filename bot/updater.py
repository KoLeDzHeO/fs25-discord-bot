...
async def update_message(bot: discord.Client):
    ...
    async with aiohttp.ClientSession() as session:
        while not bot.is_closed():
            ...
            # Лог для проверки наличия данных
            print(f"[DEBUG] Статусы: stats={bool(stats_xml)}, vehicles={bool(vehicles_xml)}, economy={bool(economy_xml)}, careerFTP={bool(career_ftp)}, farmlandFTP={bool(farmland_ftp)}")
            
            if all([stats_xml, vehicles_xml, economy_xml, career_ftp, farmland_ftp]):
                data = parse_all(
